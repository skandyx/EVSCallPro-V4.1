// FIX: Create content for IvrFeature.tsx to resolve module error.
import React, { useState } from 'react';
import type { Feature, IvrFlow } from '../types';
                    ...
                    {ivrFlows.map(flow => (
                        <li key={flow.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/50 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400">{t('ivrFeature.nodeCount', { count: flow.nodes.length })}</p>
                            </div>
